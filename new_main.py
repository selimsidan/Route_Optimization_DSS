...
class AdvancedVRPSolver:
    ...
    def solve_vrp_milp(self, vehicles_df, forbidden_groups=None, time_limit=None, last_mile_cost_per_km=1.0):
        """
        Revize edilmiş MILP tabanlı VRP çözümü:
        - Tek depo, çok locker
        - Müşteriler:
            * last_feet: Doğrudan ziyaret edilmeli.
            * last_mile: Doğrudan ziyaret edilmemeli, talepleri en yakın locker'a aktarılmalıdır.
        - M_last_feet: last_feet müşteri düğümleri.
        - M_locker: adjusted demand > 0 olan locker düğümleri (last_mile taleplerinin toplandığı noktalar).
        - M_all = M_last_feet ∪ M_locker: Ziyaret, akış, kapasite (MTZ) kısıtları bu küme üzerinden uygulanır.
        - Ek olarak: En az bir araç kullanılmalı.
        
        Yeni eklenen parametre:
        - last_mile_cost_per_km: Her last mile müşterisinin locker’a gidiş mesafesinin km başına maliyeti.
        """
        ...
        N = list(range(self.data.shape[0]))  # Tüm düğümler (0..n-1)
        ...
        # ---------------------
        # 3) Last_mile Taleplerinin Locker'a Aktarılması ve Ek Maliyet Hesaplanması
        locker_extra_demand = [0] * len(N)  # Her locker için ekstra talep
        last_mile_assignments = {}  # Her last mile müşterisi için atanan locker
        total_last_mile_cost = 0.0   # Toplam last mile ek maliyeti
        for lm in M_last_mile:
            # En yakın locker'ı belirle:
            nearest_locker = min(L, key=lambda lk: self.dist_matrix[lm][lk])
            locker_extra_demand[nearest_locker] += self.demands[lm]
            last_mile_assignments[lm] = nearest_locker
            total_last_mile_cost += last_mile_cost_per_km * self.dist_matrix[lm][nearest_locker]
        
        # ---------------------
        # 4) Adjusted Demands Hesaplanması
        adjusted_demands = [self.demands[i] for i in range(len(N))]
        for lk in L:
            adjusted_demands[lk] += locker_extra_demand[lk]
        
        # ---------------------
        # 5) Ziyaret Edilecek Düğümler (M_all)
        M_locker = L[:]  # Tüm locker’lar
        M_all = M_last_feet + M_locker
        if not M_all:
            raise ValueError("Ziyaret edilecek müşteri veya locker düğümü bulunamadı! (M_all boş)")
        
        # 6) MILP Modelinin Oluşturulması
        prob = pulp.LpProblem("SingleDepot_VRP_LastMileFeet", pulp.LpMinimize)
        
        # ---------------------
        # 7) Karar Değişkenlerinin Tanımlanması
        x = {}
        for i in N:
            for j in N:
                if i == j:
        ...
        # En az bir araç kullanılmalı:
        prob += pulp.lpSum(y[v] for v in vehicle_ids) >= 1, "AtLeastOneVehicle"
        
        # ---------------------
        # 8) MTZ Değişkenlerinin Tanımlanması (Kapasite/Flow)
        u = {}
        for i in M_all:
            for v in vehicle_ids:
                u[i, v] = pulp.LpVariable(f"u_{i}_{v}", lowBound=adjusted_demands[i])
                prob += u[i, v] <= vehicles[v]['capacity'], f"LoadMax_{i}_{v}"
        
        # ---------------------
        # 9) Amaç Fonksiyonu: Araçların sabit maliyeti ve km maliyetine ek olarak,
        #    last mile müşterilerinin locker’a gidiş mesafe maliyeti de eklenir.
        prob += (
            pulp.lpSum(
        ...
        # ---------------------
        # 13) Kapasite (MTZ) Kısıtları
        for v in vehicle_ids:
            for i in M_all:
                for j in M_all:
                    if i == j:
                        continue
                    prob += (u[i, v] + adjusted_demands[j] <= 
        ...
        for v in vehicle_ids:
            prob += pulp.lpSum((self.dist_matrix[i][j] / speed) * x[i, j, v]
                            for i in N for j in N if i != j) <= vehicles[v]['max_duration'], f"TimeLimit_{v}"    
        
        # ---------------------
        # 15) Forbidden Group Kısıtları (Varsa)
        for group in forbidden_groups:
            for v in vehicle_ids:
